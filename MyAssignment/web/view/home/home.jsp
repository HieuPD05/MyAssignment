...
<style>
:root{
  --base:#3b4b63; --base-strong:#334257; --base-soft:#51637d; --base-pale:#6a7d97;
  --ink:#ffffff; --muted:#e5edf7; --line:#2d394a; --accent:#facc15; --accent-2:#fde047;
...
body{ margin:0; min-height:100dvh; position:relative; font-family: Inter,"Segoe UI",Arial,sans-serif; color:var(--ink); }
...
.topbar{ position:sticky; top:0; z-index:10; background: var(--base); border-bottom:1px solid var(--line); box-shadow: var(--shadow);
  display:flex; align-items:center; justify-content:space-between; padding:12px 18px; color:var(--ink); }
.brand{display:flex;align-items:center;gap:12px;font-weight:800}
.logo{ width:28px;height:28px;border-radius:9px; background:linear-gradient(135deg,var(--base-pale),var(--base)); display:grid; place-items:center; color:#fff; }
...
.menu .logout{background:#991b1b; border-color:#7f1d1d}
.wrap{max-width:1100px;margin:44px auto;padding:0 20px;display:grid;gap:18px}
.hero{ background: var(--base); border:1px solid var(--line); border-radius:calc(var(--radius) + 2px);
  padding:26px; box-shadow:var(--shadow); display:flex; align-items:center; gap:20px; }
.avatar{ width:64px;height:64px;border-radius:16px; background:var(--base-strong); display:grid; place-items:center; border:1px solid #2b3748; }
...
.card{ grid-column:span 4; background: var(--base); border:1px solid var(--line); border-radius: var(--radius);
  padding:20px; box-shadow:var(--shadow); display:flex; gap:14px; align-items:flex-start; min-height:96px; }
.card:hover{ transform:translateY(-3px); border-color:#51637d; background:var(--base-strong);}
.icon{ width:44px;height:44px;border-radius:12px; background:linear-gradient(180deg,var(--base-pale),var(--base-soft)); border:1px solid #44556f; display:grid; place-items:center; }
.card h3{margin:2px 0 10px; font-size:18px}
.card a{ text-decoration:none; color:#fde68a; font-weight:700}
.card a:hover{color:#fff}
@media (max-width: 980px){ .card{grid-column:span 6} }
@media (max-width: 640px){ .hero{flex-direction:column; align-items:flex-start} .card{grid-column:span 12} .menu{flex-wrap:wrap} }
</style>
</head>
<body>
<header class="topbar">
